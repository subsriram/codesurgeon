...
| `CS_REVERSE_EXPAND_STRATEGY` | `codesurgeon`, `codesurgeon-mcp` | Selects the reverse/forward-edge expansion ranking variant: `none`, `v0`, `v1a`, `v1b`, `v1ab`, `v2` (default — current main behaviour), `v3a`, `v3b`. Used by the cs-benchmark `reverse_expand_panel` to ablate ranking strategies; see [issue #69](https://github.com/subsriram/codesurgeon/issues/69) and `crates/cs-core/src/ranking.rs::ReverseExpandStrategy`. |
...
| `CS_REVERSE_EXPAND_TOTAL_BUDGET` | `codesurgeon`, `codesurgeon-mcp` | Override the best-first walker's output cap (default `40`). Cs-benchmark uses this to ablate budget without rebuilding the binary; see [issue #96](https://github.com/subsriram/codesurgeon/issues/96). |
| `CS_REVERSE_EXPAND_EXPAND_BUDGET` | `codesurgeon`, `codesurgeon-mcp` | Override the best-first walker's graph-expansion cap (default `200`). Pair with `CS_REVERSE_EXPAND_TOTAL_BUDGET` when budget-sweeping. |

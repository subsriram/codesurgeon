...
| `CS_EXPAND_STRATEGY` | `codesurgeon`, `codesurgeon-mcp` | Selects the anchor-expansion ranking variant — applies to both forward and reverse walks: `none`, `v0`, `v1a`, `v1b`, `v1ab`, `v2` (default — current main behaviour), `v3a`, `v3b`. Used by the cs-benchmark `reverse_expand_panel` to ablate ranking strategies; see [issue #69](https://github.com/subsriram/codesurgeon/issues/69) and `crates/cs-core/src/ranking.rs::ExpandStrategy`. The deprecated alias `CS_REVERSE_EXPAND_STRATEGY` is still accepted (with a warn) and will be removed. |
| `CS_EXPAND_DIRECTION` | `codesurgeon`, `codesurgeon-mcp` | Selects walk direction for anchor expansion: `auto` (default — per-anchor classifier reads kind + fan-out ratio), `forward` (walk callees), `reverse` (walk callers — original #67 behaviour), `both` (split budget). See [issue #95](https://github.com/subsriram/codesurgeon/issues/95). |
| `CS_EXPAND_TOTAL_BUDGET` | `codesurgeon`, `codesurgeon-mcp` | Override the best-first walker's output cap (default `40`) — applies to both forward and reverse walks. Cs-benchmark uses this to ablate budget without rebuilding the binary; see [issue #96](https://github.com/subsriram/codesurgeon/issues/96). Deprecated alias: `CS_REVERSE_EXPAND_TOTAL_BUDGET`. |
| `CS_EXPAND_GRAPH_BUDGET` | `codesurgeon`, `codesurgeon-mcp` | Override the best-first walker's graph-expansion cap (default `200`). Pair with `CS_EXPAND_TOTAL_BUDGET` when budget-sweeping. Deprecated alias: `CS_REVERSE_EXPAND_EXPAND_BUDGET`. |

...
pub struct Anchors {
    ...
    /// traceback shortcut in `engine.rs`: every frame is pinned as a
    /// direct pivot candidate, bypassing the walk-direction question.
    /// Issue #95 layer 1.
...
pub fn extract(query: &str) -> Anchors {
    ...
    for cap in traceback_frame_re().captures_iter(query) {
        ...
        if let Some(func) = cap.get(2) {
            ...
            // can use it as a direct pivot candidate (issue #95 layer 1).
